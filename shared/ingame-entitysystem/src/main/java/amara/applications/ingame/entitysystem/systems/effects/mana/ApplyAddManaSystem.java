...
import amara.applications.ingame.entitysystem.components.effects.mana.AddManaComponent;
...
public class ApplyAddManaSystem implements EntitySystem {
    ...
    public void update(EntityWorld entityWorld, float deltaSeconds) {
        for (int effectImpactEntity : entityWorld.getEntitiesWithAll(ApplyEffectImpactComponent.class, AddManaComponent.class)) {
            int targetEntity = entityWorld.getComponent(effectImpactEntity, ApplyEffectImpactComponent.class).getTargetEntity();
            ManaComponent manaComponent = entityWorld.getComponent(targetEntity, ManaComponent.class);
            if (manaComponent != null) {
                float newMana = (manaComponent.getValue() + entityWorld.getComponent(effectImpactEntity, AddManaComponent.class).getValue());
